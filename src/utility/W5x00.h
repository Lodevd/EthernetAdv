...
public:
  ...
  //W5x00Class(SPIClass &spi, uint8_t sspin, uint8_t sockNum);

  virtual uint8_t init(void) = 0;

  virtual W5x00Linkstatus getLinkStatus() = 0;

  virtual void beginTransaction() = 0;

  virtual uint16_t SBASE(uint8_t socknum) = 0;

  virtual uint16_t RBASE(uint8_t socknum) = 0;

  virtual const bool hasOffsetAddressMapping() = 0;
  ...
  // Generic for W5x00 Classes (already implemented)
  // -------------------------
public:
  ...
  inline uint16_t getLocalPort(SOCKET s) { return readSnPORT(s); }
  inline IPAddress getRemoteIp(SOCKET s) { uint8_t i[4]; readSnDIPR(s, i); return IPAddress(i); }
  inline uint16_t getRemotePort(SOCKET s) { return readSnDPORT(s); }

  void endTransaction();

  uint8_t maxSockNum() { return _maxSockNum; }
  ...
  uint16_t SSIZE = 2048;                //Will changed during init depending on the number of sockets
  uint16_t SMASK = 0x07FF;              //Will changed during init depending on the number of sockets
...
#undef __GP_REGISTER_N
  ...
  // ----------------------
public:
  inline uint16_t CH_BASE(void) {
    //if (chip == 55) return 0x1000;
    //if (chip == 52) return 0x4000;
    //if (chip == 51) return 0x0400;
